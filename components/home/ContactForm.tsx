  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" id='contact'>
      ...
      {submitStatus === 'success' && (
        <p className="mt-3 text-sm text-green-600">Mensaje enviado con éxito. Gracias por contactarnos.</p>
      )}
      {submitStatus === 'error' && (
        <p className="mt-3 text-sm text-red-600">Hubo un problema al enviar tu mensaje. Por favor, inténtalo de nuevo.</p>
      )}
    </form>
  )
}
